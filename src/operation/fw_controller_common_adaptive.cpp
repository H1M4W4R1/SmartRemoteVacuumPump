...
static int32_t fwControllerAdaptiveError(const FwConfig *config, int32_t pressureHpa, int32_t targetHpa)
{
    int32_t sign = 1L;
    ...
    return sign * (targetHpa - pressureHpa);
}

static bool fwControllerAdaptiveReset(const FwConfig *config, int32_t pressureHpa, int32_t targetHpa)
{
    assert(config != nullptr);
    assert(pressureHpa > -2000L);
    if (config == nullptr) {
    ...
    g_adaptiveLastErrorHpa = fwControllerAdaptiveError(config, pressureHpa, targetHpa);
    ...
    g_adaptiveLastTargetHpa = targetHpa;
    return true;
}

static bool fwControllerAdaptiveRefresh(const FwConfig *config, int32_t pressureHpa, int32_t targetHpa)
{
    bool changed = false;
    assert(config != nullptr);
    assert(config->mode <= FW_MODE_CURRENT_ADAPTIVE);
    if (config == nullptr) {
        return false;
    }
    ...
    if (changed) {
        return fwControllerAdaptiveReset(config, pressureHpa, targetHpa);
    }
    return true;
}
    ...
    int32_t errorHpa, uint32_t elapsedMs)
{
    ...
    return (config->mode == FW_MODE_TARGET_ADAPTIVE) || (config->mode == FW_MODE_CURRENT_ADAPTIVE);
}
...
static bool fwControllerAdaptivePollAt(FwConfig *config, FwSession *session, int32_t pressureHpa,
    int32_t targetHpa)
{
    ...
    ok = fwControllerAdaptiveRefresh(config, pressureHpa, targetHpa);
    if (!ok) {
        return false;
    }
    ...
    errorHpa = fwControllerAdaptiveError(config, pressureHpa, targetHpa);
...
bool fwControllerCommonAdaptiveTargetPoll(FwConfig *config, FwSession *session, int32_t pressureHpa)
{
    ...
    return fwControllerAdaptivePollAt(config, session, pressureHpa, config->targetPressureHpa);
}
...
bool fwControllerCommonAdaptiveCurrentPoll(FwConfig *config, FwSession *session, int32_t pressureHpa)
{
    ...
    return fwControllerAdaptivePollAt(config, session, pressureHpa, session->currentHoldPressureHpa);
}
